...
--[[
    KronosV2.3.lua
    Reference-locked native Roblox UI library refined around denser target-like
    composition, integrated header geometry, responsive row allocation, and a
    restrained semantic surface system. All controllers and the optional showcase remain self-contained;
...
local DefaultAppearance = {
    ...
    AcrylicEnabled = true,
    AcrylicIntensity = 0.56,
    BorderIntensity = 0.9,
    SurfaceContrast = 1.13,
    ReducedMotion = false,
...
local Theme: ThemeMap = {
    -- V2.3 uses a narrower, softer violet-black ladder. Adjacent regions remain
    -- readable without oversized cards, luminous rails, or a bright shell frame.
    Background = Color3.fromRGB(10, 10, 13),
    BackgroundSoft = Color3.fromRGB(14, 14, 18),
    Surface = Color3.fromRGB(17, 17, 22),
    Surface2 = Color3.fromRGB(21, 21, 27),
    Surface3 = Color3.fromRGB(26, 26, 33),
    ElevatedSurface = Color3.fromRGB(24, 24, 31),
    SurfaceHover = Color3.fromRGB(31, 30, 39),
    HoverSurface = Color3.fromRGB(31, 30, 39),
    PressedSurface = Color3.fromRGB(34, 32, 44),
    Stroke = Color3.fromRGB(58, 56, 69),
    StrokeSoft = Color3.fromRGB(41, 40, 50),
    Border = Color3.fromRGB(54, 52, 65),
    Divider = Color3.fromRGB(38, 37, 46),
    Text = Color3.fromRGB(239, 238, 244),
    PrimaryText = Color3.fromRGB(239, 238, 244),
    SubText = Color3.fromRGB(177, 174, 187),
    SecondaryText = Color3.fromRGB(177, 174, 187),
    Muted = Color3.fromRGB(114, 111, 124),
    Disabled = Color3.fromRGB(59, 58, 68),
    DisabledText = Color3.fromRGB(116, 113, 125),
    Accent = Color3.fromRGB(137, 121, 241),
    AccentDark = Color3.fromRGB(88, 78, 183),
    AccentSoft = Color3.fromRGB(177, 166, 249),
    AccentHover = Color3.fromRGB(149, 135, 246),
    AccentPressed = Color3.fromRGB(113, 99, 218),
    ...
    InnerHighlight = Color3.fromRGB(89, 84, 108),
    ScrollTrack = Color3.fromRGB(27, 26, 33),
    AcrylicTint = Color3.fromRGB(21, 19, 28),
}
...
-- V2.3 treats the objective viewport ratios as structural constraints rather
...
local TargetDesign = {
    ...
    Window = {
        LogicalSize = Vector2.new(1020, 590),
        DesktopLogicalSize = Vector2.new(1480, 835),
        Aspect = 1.72,
        MinimumAspect = 1.5,
        MaximumAspect = 1.98,
        Center = Vector2.new(0.5, 0.51),
        Radius = 9,
        WideHeightRatio = 0.89,
        MediumHeightRatio = 0.91,
        CompactHeightRatio = 0.93,
        LandscapeHeightRatio = 0.95,
        WideWidthRatio = 0.97,
        MediumWidthRatio = 0.96,
        CompactWidthRatio = 0.955,
        LandscapeWidthRatio = 0.965,
    ...
    Content = {
        MaximumWidth = 1460,
        SingleColumnMaximum = 640,
        MinimumSection = 220,
        ColumnGap = 12,
        SectionGap = 7,
        HorizontalPadding = 12,
        VerticalPadding = 8,
        PageContextHeight = 44,
        SubtabInset = 0,
    },
    Control = {
        RowHeight = 35,
    ...
    Scrollbar = {
        ...
        ActiveWidth = 8,
        SidebarIdleWidth = 3,
        SidebarActiveWidth = 5,
        PopupIdleWidth = 4,
        PopupActiveWidth = 6,
        MinimumThumb = 56,
        SidebarMinimumThumb = 44,
        PopupMinimumThumb = 34,
        HorizontalMinimumThumb = 52,
        TrackInset = 2,
        Margin = 6,
        SidebarMargin = 4,
        PopupMargin = 4,
        HorizontalMargin = 6,
        IdleTransparency = 0.28,
        SidebarIdleTransparency = 0.74,
        PopupIdleTransparency = 0.44,
        ActiveTransparency = 0.06,
        TrackIdleTransparency = 0.9,
        SidebarTrackIdleTransparency = 0.98,
        PopupTrackIdleTransparency = 0.95,
        TrackActiveTransparency = 0.74,
        FadeDelay = 0.9,
        IdleDelay = 0.9,
        WheelStep = 46,
    },
    ...
    Widget = {
        Target = Vector2.new(242, 72),
        TargetWidth = 242,
        TargetHeight = 72,
        Keybind = Vector2.new(224, 60),
        KeybindWidth = 224,
        KeybindHeight = 60,
        StatusWidth = 170,
        StatusHeight = 30,
...
assert(
    type(TargetDesign.Calibration) == "table"
        ...
        and type(TargetDesign.Typography) == "table"
...
local Metrics = {
    -- Target-derived semantic measurements from Image 2.
    ReferenceViewport = Vector2.new(897, 610),
    ReferenceWindow = Vector2.new(880, 529),
    ReferenceAspect = 880 / 529,
...
local BorderStyles = {
    ...
    Shell = { Transparency = 0.42, Thickness = 1 },
    MainShellOuter = { Transparency = 0.3, Thickness = 1 },
    InnerHighlight = { Transparency = 0.82, Thickness = 1 },
    ...
    ControlFocused = { Transparency = 0.24, Thickness = 1 },
...
function ResponsiveController:GetRawDensity(viewport: Vector2?): number
    ...
    -- V2.3 separates readable typography from oversized card geometry. Density
    -- remains near one on desktop, while short landscape captures receive a
    -- restrained reduction that exposes more rows without shrinking the shell.
    if portrait then
        if size.X <= 390 then
            return 0.88
        elseif size.X <= 520 then
            return 0.92
        end
        return 0.96
    end
    ...
    elseif size.Y <= 760 then
        return 0.94
    elseif size.X < 1440 or size.Y < 900 then
        return 0.97
    end
    return 1
end
...
function ResponsiveController:GetLayoutMode(viewport: Vector2, safeWidth: number, safeHeight: number): string
    ...
    if safeWidth < 920 or safeHeight < 540 then
        return "Compact"
    end
    if safeWidth < 1500 or safeHeight < 840 then
        return "Medium"
    end
    return "Wide"
end
...
function ResponsiveController:CalculateWindowSize(
    ...
    local explicitAspect = math.clamp(
        baseWidth / baseHeight,
        TargetDesign.Window.MinimumAspect,
        TargetDesign.Window.MaximumAspect
    )
    local aspect = (window.HasExplicitWidth or window.HasExplicitHeight) and explicitAspect
        or TargetDesign.Window.Aspect
    if mode == "MobileLandscape" and not window.HasExplicitWidth and not window.HasExplicitHeight then
        -- Wide touch captures need additional horizontal occupancy without
        -- sacrificing the target-like vertical presence.
        aspect = math.clamp(
            safeWidth / math.max(safeHeight, 1) * 0.9,
            TargetDesign.Window.Aspect,
            TargetDesign.Window.MaximumAspect
        )
    end
    local width: number
    ...
    if mode ~= "Portrait" then
        ...
        if currentAspect > aspect then
            ...
            if targetHeight <= safeHeight then
                height = targetHeight
            else
                height = safeHeight
                width = height * aspect
            end
        elseif currentAspect < aspect then
            local targetWidth = height * aspect
...
function ScrollbarController:Wake(entry: AnyTable, hold: boolean?)
    ...
    tween(entry.Track, {
        BackgroundTransparency = entry.TrackActiveTransparency,
    }, Motion.ScrollbarActivate)
    tween(entry.Thumb, {
        BackgroundTransparency = entry.ActiveTransparency,
    }, Motion.ScrollbarActivate)
    if hold then
        return
    end
    task.delay(TargetDesign.Scrollbar.IdleDelay, function()
        if entry.IdleGeneration == generation and not entry.Hovered and not entry.Dragging and entry.Track.Parent then
            tween(entry.Track, {
                BackgroundTransparency = entry.TrackIdleTransparency,
            }, Motion.ScrollbarIdle)
            tween(entry.Thumb, {
                BackgroundTransparency = entry.IdleTransparency,
            }, Motion.ScrollbarIdle)
        end
    end)
end
...
function ScrollbarController:Attach(scroller: ScrollingFrame): AnyTable?
    ...
    local role = tostring(scroller:GetAttribute("KronosScrollbarRole") or scroller.Name)
    local sidebarRole = role == "Navigation" or role == "Sidebar"
    local loweredRole = string.lower(role)
    local popupRole = not sidebarRole and (
        string.find(loweredRole, "popup", 1, true) ~= nil
        or string.find(loweredRole, "dropdown", 1, true) ~= nil
        or string.find(loweredRole, "context", 1, true) ~= nil
        or string.find(loweredRole, "option", 1, true) ~= nil
    )
    local entry: AnyTable = {
        Scroller = scroller,
        Connections = {},
        Horizontal = horizontal,
        Role = sidebarRole and "Sidebar" or (popupRole and "Popup" or "Content"),
        Margin = sidebarRole and TargetDesign.Scrollbar.SidebarMargin
            or (popupRole and TargetDesign.Scrollbar.PopupMargin or TargetDesign.Scrollbar.Margin),
        IdleWidth = sidebarRole and TargetDesign.Scrollbar.SidebarIdleWidth
            or (popupRole and TargetDesign.Scrollbar.PopupIdleWidth or TargetDesign.Scrollbar.IdleWidth),
        ActiveWidth = sidebarRole and TargetDesign.Scrollbar.SidebarActiveWidth
            or (popupRole and TargetDesign.Scrollbar.PopupActiveWidth or TargetDesign.Scrollbar.ActiveWidth),
        MinimumThumb = horizontal and TargetDesign.Scrollbar.HorizontalMinimumThumb
            or (sidebarRole and TargetDesign.Scrollbar.SidebarMinimumThumb
                or (popupRole and TargetDesign.Scrollbar.PopupMinimumThumb or TargetDesign.Scrollbar.MinimumThumb)),
        IdleTransparency = sidebarRole and TargetDesign.Scrollbar.SidebarIdleTransparency
            or (popupRole and TargetDesign.Scrollbar.PopupIdleTransparency or TargetDesign.Scrollbar.IdleTransparency),
        ActiveTransparency = TargetDesign.Scrollbar.ActiveTransparency,
        TrackIdleTransparency = sidebarRole and TargetDesign.Scrollbar.SidebarTrackIdleTransparency
            or (popupRole and TargetDesign.Scrollbar.PopupTrackIdleTransparency or TargetDesign.Scrollbar.TrackIdleTransparency),
        TrackActiveTransparency = TargetDesign.Scrollbar.TrackActiveTransparency,
        Hovered = false,
        Dragging = false,
    }
    ...
    corner(track, 3)
    corner(thumb, 3)
    local trackStroke = stroke(track, Theme.Border, 0.9, 1, "ScrollTrack")
    local thumbStroke = stroke(thumb, Theme.AccentSoft, 0.88, 1, "AcrylicHighlight")
    ThemeController:Bind(track, "BackgroundColor3", "ScrollTrack")
    ThemeController:Bind(trackStroke, "Color", "Border")
    ThemeController:Bind(thumb, "BackgroundColor3", "Accent")
    ThemeController:Bind(thumbStroke, "Color", "AccentSoft")
    entry.Track = track
    entry.Thumb = thumb
        ...
        addConnection(
            ...
            scroller:GetPropertyChangedSignal(propertyName):Connect(function()
                if propertyName == "ScrollingDirection" then
                    ...
                    entry.MinimumThumb = entry.Horizontal and TargetDesign.Scrollbar.HorizontalMinimumThumb
                        or (entry.Role == "Sidebar" and TargetDesign.Scrollbar.SidebarMinimumThumb
                            or (entry.Role == "Popup" and TargetDesign.Scrollbar.PopupMinimumThumb
                                or TargetDesign.Scrollbar.MinimumThumb))
                    entry.Margin = entry.Horizontal and TargetDesign.Scrollbar.HorizontalMargin
                        or (entry.Role == "Sidebar" and TargetDesign.Scrollbar.SidebarMargin
                            or (entry.Role == "Popup" and TargetDesign.Scrollbar.PopupMargin
                                or TargetDesign.Scrollbar.Margin))
                end
                refresh()
...
local function makeControlRow(section, titleText, description, height, iconOptions: IconOptions?)
    ...
    local horizontalPadding = TargetDesign.Control.HorizontalPadding
    local configTable = iconOptions :: AnyTable?
    local holderRatio = math.clamp(
        finiteNumber(configTable and (configTable.HolderRatio or configTable.ControlRatio), TargetDesign.Control.HolderRatio),
        0.34,
        0.58
    )
    local titleLength = utf8.len(tostring(titleText or "")) or #tostring(titleText or "")
    local candidateValue = configTable
        and (configTable.Default or configTable.ButtonText or configTable.Placeholder or configTable.Text)
    local valueLength = candidateValue ~= nil and (utf8.len(tostring(candidateValue)) or #tostring(candidateValue)) or 0
    if titleLength >= 13 and valueLength <= 9 then
        holderRatio = math.min(holderRatio, 0.39)
    elseif valueLength >= 14 then
        holderRatio = math.max(holderRatio, 0.48)
    end
    ...
    corner(row, 2)
    local rowStroke = stroke(row, Theme.StrokeSoft, 0.985, 1, "Quiet")
    ...
    local rowDivider = create("Frame", {
        ...
        BorderSizePixel = 0,
        AnchorPoint = Vector2.new(0, 1),
        Position = UDim2.new(0, horizontalPadding, 1, 0),
        Size = UDim2.new(1, -horizontalPadding * 2, 0, 1),
        ZIndex = 1,
        Parent = row,
    }) :: Frame
    ThemeController:Bind(rowDivider, "BackgroundColor3", "Divider")

    local titleOffset = horizontalPadding
    ...
    if iconOptions and iconOptions.Icon then
        ...
        if rowIcon then
            ...
            rowIcon.Position = UDim2.new(0, horizontalPadding, 0.5, 0)
            rowIcon.ZIndex = 2
            titleOffset = horizontalPadding + rowIcon.Size.X.Offset + 9
        end
    end
    local labelWidth = math.max(1 - holderRatio, 0.42)
    local title = makeText(row, titleText or "Control", TargetDesign.Typography.Control, Theme.Text, "medium")
    title.Name = "ControlTitle"
    title.Position = UDim2.fromOffset(titleOffset, description and 5 or 0)
    title.Size = UDim2.new(labelWidth, -titleOffset - 4, 0, 17)
    ...
    local descLabel
    if description then
        descLabel = makeText(row, description, TargetDesign.Typography.Description, Theme.Muted)
        descLabel.Position = UDim2.fromOffset(titleOffset, 22)
        descLabel.Size = UDim2.new(labelWidth, -titleOffset - 4, 0, 16)
    ...
    local holder = create("Frame", {
        ...
        Position = UDim2.new(1, -horizontalPadding, 0.5, 0),
        Size = UDim2.new(holderRatio, -2, 1, -6),
        Parent = row,
    })
    ...
    addConnection(
        ...
        hover.MouseEnter:Connect(function()
            if not row:GetAttribute("KronosDisabled") then
                tween(row, { BackgroundTransparency = 0.86 }, Motion.HoverIn)
                tween(rowStroke, { Transparency = BorderController:Resolve(0.9, "ControlHover") }, Motion.HoverIn)
            end
        end)
    )
    addConnection(
        ...
        hover.MouseLeave:Connect(function()
            tween(row, { BackgroundTransparency = 0.965 }, Motion.HoverOut)
            tween(rowStroke, { Transparency = BorderController:Resolve(0.985, "Quiet") }, Motion.HoverOut)
        end)
    )
...
function Section:CreateToggle(id: any, config: ComponentOptions?): AnyTable
    ...
    config = config or {}
    local row, holder, titleLabel, descriptionLabel, hitbox =
        makeControlRow(self, config.Name or config.Title or id or "Toggle", config.Description, nil, config)
    -- Target-style checkbox rows place simple toggles before the label. Icon-led
    -- or explicitly trailing toggles retain the mature right-side composition.
    local leadingCheckbox = config.Icon == nil and config.Trailing ~= true and config.RightAligned ~= true
    if leadingCheckbox then
        holder.Visible = false
        local leadingOffset = TargetDesign.Control.HorizontalPadding
            + TargetDesign.Control.Toggle
            + 10
        if config.Description then
            titleLabel.AnchorPoint = Vector2.new(0, 0)
            titleLabel.Position = UDim2.fromOffset(leadingOffset, 7)
        else
            titleLabel.AnchorPoint = Vector2.new(0, 0.5)
            titleLabel.Position = UDim2.new(0, leadingOffset, 0.5, 0)
        end
        titleLabel.Size = UDim2.new(1, -leadingOffset - TargetDesign.Control.HorizontalPadding, 0, 18)
        if descriptionLabel then
            descriptionLabel.Position = UDim2.fromOffset(leadingOffset, 26)
            descriptionLabel.Size = UDim2.new(
                1,
                -leadingOffset - TargetDesign.Control.HorizontalPadding,
                0,
                17
            )
        end
    ...
    local box = create("TextButton", {
        ...
        BackgroundTransparency = toggle.Value and 0.02 or 0.22,
        BorderSizePixel = 0,
        Text = "",
        AutoButtonColor = false,
        AnchorPoint = leadingCheckbox and Vector2.new(0, 0.5) or Vector2.new(1, 0.5),
        Position = leadingCheckbox
            and UDim2.new(0, TargetDesign.Control.HorizontalPadding, 0.5, 0)
            or UDim2.new(1, -1, 0.5, 0),
        Size = UDim2.fromOffset(TargetDesign.Control.Toggle, TargetDesign.Control.Toggle),
        Parent = leadingCheckbox and row or holder,
        ZIndex = 6,
    }) :: TextButton
    corner(box, 3)
    local boxStroke = stroke(box, toggle.Value and Theme.AccentSoft or Theme.Border, toggle.Value and 0.28 or 0.68, 1)
    local check = makeIcon(box, { Icon = "check", IconSize = 11 }, "White")
    if check then
    ...
    local function render(value: boolean, instant: boolean?)
        ...
        AnimationController:Tween(box, {
            ...
            BackgroundTransparency = value and 0.02 or 0.22,
        }, duration)
        AnimationController:Tween(boxStroke, {
            Color = value and Theme.AccentSoft or Theme.Border,
            Transparency = value and 0.28 or 0.68,
        }, duration)
        if check then
...
function Section:CreateSlider(id: any, config: ComponentOptions?): AnyTable
    ...
    local row, holder, titleLabel = makeControlRow(
        ...
        config.Description,
        config.Description and Metrics.DescriptionRow or Metrics.Row,
        config
    )
    holder.Size = UDim2.new(TargetDesign.Control.HolderRatio, -4, 1, -10)
    ...
    local valueLabel = makeText(holder, "", TargetDesign.Typography.Value, Theme.SubText, "bold")
    valueLabel.AnchorPoint = Vector2.new(1, 0)
    valueLabel.Position = UDim2.new(1, 0, 0, 0)
    valueLabel.Size = UDim2.fromOffset(82, 16)
    ...
    corner(knob, TargetDesign.Control.SliderThumb)
    ...
    addConnection(
        ...
        hit.InputBegan:Connect(function(input)
            ...
            then
                ...
                dragging = true
                update(input)
                local size = ResponsiveController:Scale(self.Window, TargetDesign.Control.SliderThumb + 3)
                tween(knob, { Size = UDim2.fromOffset(size, size) }, Motion.Press)
            end
        end)
    )
...
function Section:CreateInput(id: any, config: ComponentOptions?): AnyTable
    ...
    local box = create("TextBox", {
        ...
        TextSize = TargetDesign.Typography.Value,
    ...
    corner(box, TargetDesign.Control.Radius)
    local boxStroke = stroke(box, Theme.Border, 0.76, 1, "Control")
    ...
    addConnection(
        ...
        box.FocusLost:Connect(function(enterPressed)
            tween(box, { BackgroundTransparency = 0.1 }, Motion.Hover)
...
function Section:CreateDropdown(id: any, config: ComponentOptions?): AnyTable
    ...
    corner(button, TargetDesign.Control.Radius)
    local buttonStroke = stroke(button, Theme.Border, 0.76, 1, "Control")
    local label = makeText(button, "", TargetDesign.Typography.Value, Theme.SubText, "medium")
    label.Position = UDim2.fromOffset(10, 0)
    label.Size = UDim2.new(1, -30, 1, 0)
    label.ZIndex = 7
    local arrow = makeIcon(button, { Icon = "chevron-down", IconSize = 11 }, "Muted")
    if arrow then
    ...
    local function openMenu()
        ...
        local searchable = config.Search == true or #options > TargetDesign.Popup.MaximumVisibleOptions
        local visibleRows = math.clamp(#options, 1, TargetDesign.Popup.MaximumVisibleOptions)
        local popupHeight = visibleRows * TargetDesign.Popup.OptionHeight
            + math.max(visibleRows - 1, 0) * 2
            + TargetDesign.Popup.Padding * 2
            + (searchable and TargetDesign.Popup.SearchHeight + 6 or 0)
        local minimumPopupHeight = TargetDesign.Popup.OptionHeight + TargetDesign.Popup.Padding * 2
        local maximumPopupHeight = TargetDesign.Popup.MaximumVisibleOptions * TargetDesign.Popup.OptionHeight
            + TargetDesign.Popup.Padding * 2
            + (searchable and TargetDesign.Popup.SearchHeight + 6 or 0)
        popupHeight = math.clamp(popupHeight, minimumPopupHeight, maximumPopupHeight)
        ...
        padding(popup, TargetDesign.Popup.Padding, TargetDesign.Popup.Padding, TargetDesign.Popup.Padding, TargetDesign.Popup.Padding)
        ...
        if searchable then
            ...
            corner(searchBox, TargetDesign.Control.Radius)
            stroke(searchBox, Theme.Border, 0.5, 1, "Focus")
            padding(searchBox, 10, 0, 10, 0)
            topOffset = TargetDesign.Popup.SearchHeight + 6
        end
        ...
        local optionLayout = list(listFrame, Enum.FillDirection.Vertical, 2)
        ...
        local empty = makeText(scroll, "No options found", TargetDesign.Typography.Value, Theme.Muted)
            ...
            for _, option in ipairs(options) do
                ...
                if query == "" or string.find(string.lower(optionText), query, 1, true) then
                    ...
                    local item = create("TextButton", {
                        ...
                        BackgroundTransparency = selected and 0.42 or 0.78,
                        BorderSizePixel = 0,
                        Text = "",
                        AutoButtonColor = false,
                    ...
                    corner(item, TargetDesign.Control.Radius)
                    local indicator = create("Frame", {
                        BackgroundColor3 = Theme.Accent,
                        BackgroundTransparency = selected and 0 or 1,
                        BorderSizePixel = 0,
                        Position = UDim2.fromOffset(1, 7),
                        Size = UDim2.fromOffset(3, TargetDesign.Popup.OptionHeight - 14),
                        ZIndex = 705,
                        Parent = item,
                    }) :: Frame
                    ...
                    local optionIconName = type(config.OptionIcons) == "table"
                        ...
                        or nil
                    local optionOffset = 12
                    local optionIcon =
                        makeIcon(item, { Icon = optionIconName, IconSize = 12 }, selected and "Accent" or "Muted")
                    if optionIcon then
                        optionIcon.AnchorPoint = Vector2.new(0, 0.5)
                        optionIcon.Position = UDim2.new(0, 12, 0.5, 0)
                    ...
                    if dropdown.Multi then
                        ...
                        if selected then
                            local check = makeIcon(checkBox, { Icon = "check", IconSize = 11 }, "White")
                            if check then
                                check.AnchorPoint = Vector2.new(0.5, 0.5)
                                check.Position = UDim2.fromScale(0.5, 0.5)
                                check.ZIndex = 706
                    ...
                    table.insert(
                        ...
                        item.MouseEnter:Connect(function()
                            tween(item, { BackgroundTransparency = selected and 0.32 or 0.64 }, Motion.Hover)
                        end)
                    )
                    table.insert(
                        optionConnections,
                        item.MouseLeave:Connect(function()
                            tween(item, { BackgroundTransparency = selected and 0.42 or 0.78 }, Motion.Hover)
                        end)
                    )
                    table.insert(
                        optionConnections,
    ...
    addConnection(
        ...
        button.MouseEnter:Connect(function()
            if not dropdown.Disabled then
                tween(button, { BackgroundTransparency = 0.08 }, Motion.Hover)
    ...
    addConnection(
        ...
        button.MouseLeave:Connect(function()
            tween(button, { BackgroundTransparency = 0.18 }, Motion.Hover)
...
function Section:CreateButton(id: any, config: ComponentOptions?): AnyTable
    ...
    local label = makeText(button, actionText, TargetDesign.Typography.Value, config.Primary and Theme.White or Theme.SubText, "bold")
            ...
            if not control.Disabled and not control.Busy then
                tween(button, {
                    Size = UDim2.new(
                        ...
                        0,
                        ResponsiveController:Scale(self.Window, TargetDesign.Control.Row - 2)
                    ),
                }, Motion.Press)
            end
        end)
    ...
    addConnection(
        ...
        button.MouseButton1Up:Connect(function()
            tween(button, { Size = UDim2.new(1, 0, 0, ResponsiveController:Scale(self.Window, TargetDesign.Control.Row)) }, Motion.Press)
        end)
    )
    addConnection(
        control,
...
function Section:CreateLabel(id: any, config: (ComponentOptions | string)?): AnyTable
    ...
    local label = makeText(
        frame,
        textValue,
        tonumber(config.TextSize) or TargetDesign.Typography.Control,
...
function Section:CreateParagraph(id: any, config: ComponentOptions?): AnyTable
    ...
    corner(frame, 7)
    stroke(frame, Theme.Border, 0.64, 1, "Control")
    padding(frame, 11, 9, 11, 9)
    frame:SetAttribute("KronosSearch", string.lower(tostring(titleText or "") .. " " .. textValue))
    local paragraphLayout = list(frame, Enum.FillDirection.Vertical, 5)
    ...
    if titleText then
        ...
        titleLabel = makeText(heading, tostring(titleText), TargetDesign.Typography.Control, Theme.Text, "bold")
        titleLabel.Position = UDim2.fromOffset(titleOffset, 0)
        titleLabel.Size = UDim2.new(1, -titleOffset, 1, 0)
    end
    local content = makeText(frame, textValue, TargetDesign.Typography.Description, Theme.SubText)
    content.Size = UDim2.new(1, 0, 0, 36)
...
function Section:CreateDivider(id: any, config: (ComponentOptions | string)?): AnyTable
    ...
    local frame = create("Frame", {
        Name = "Divider",
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 0, (config.Name or config.Title) and 28 or 16),
        Parent = self.Content,
    }) :: Frame
    ...
    if dividerTitle then
        local measured = TextService:GetTextSize(tostring(dividerTitle), TargetDesign.Typography.Metadata, Enum.Font.GothamBold, Vector2.new(1000, 18))
        ...
        local titleHolder = create("Frame", {
            ...
            Size = UDim2.fromOffset(measured.X + iconWidth + 16, 18),
            Parent = frame,
        }) :: Frame
        ...
        titleLabel = makeText(titleHolder, tostring(dividerTitle), TargetDesign.Typography.Metadata, Theme.Muted, "bold")
...
function Section:CreateKeybind(id: any, config: ComponentOptions?): AnyTable
    ...
    holder.Size = UDim2.fromOffset(132, TargetDesign.Control.InputHeight)
    ...
    local modeButton = create("TextButton", {
        ...
        TextSize = TargetDesign.Typography.Metadata,
        AutoButtonColor = false,
        AnchorPoint = Vector2.new(1, 0.5),
        Position = UDim2.new(1, -82, 0.5, 0),
        Size = UDim2.fromOffset(28, TargetDesign.Control.InputHeight),
        ZIndex = 6,
        Parent = holder,
    }) :: TextButton
    corner(modeButton, TargetDesign.Control.Radius)
    ...
    local keyButton = create("TextButton", {
        ...
        AnchorPoint = Vector2.new(1, 0.5),
        Position = UDim2.new(1, 0, 0.5, 0),
        Size = UDim2.fromOffset(78, TargetDesign.Control.InputHeight),
        ZIndex = 6,
        Parent = holder,
    }) :: TextButton
    corner(keyButton, TargetDesign.Control.Radius)
    local keyStroke = stroke(keyButton, Theme.Border, nil, 1, "Control")
    local keyLabel = makeText(keyButton, "", TargetDesign.Typography.Value, Theme.SubText, "bold")
    ...
    addConnection(
        ...
        modeButton.Activated:Connect(function()
            ...
            local valueTitle = makeText(popup, "VALUE", TargetDesign.Typography.Metadata, Theme.Muted, "bold")
            valueTitle.Position = UDim2.fromOffset(0, 70)
            ...
            corner(showChoice, 4)
            local showLabel = makeText(showChoice, "Show in binds", TargetDesign.Typography.Value, Theme.SubText, "bold")
...
function Section:CreateColorpicker(id: any, config: ComponentOptions?): AnyTable
    ...
    corner(preview, TargetDesign.Control.Radius)
    local previewStroke = stroke(preview, Theme.Border, 0.44, 1, "ColorPicker")
    local previewLabel = makeText(preview, rgbToHex(initial), TargetDesign.Typography.Metadata, Theme.White, "bold")
    ...
    local function openPicker()
        ...
        local popupHeight = alphaEnabled and 258 or 226
        ...
        local pickerTitle =
            makeText(popup, tostring(config.PopupTitle or config.Title or "Color"), TargetDesign.Typography.Control, Theme.Text, "bold")
        pickerTitle.Size = UDim2.new(1, 0, 0, 22)
        ...
        local hueFrame = create("Frame", {
            BackgroundColor3 = Theme.White,
            BorderSizePixel = 0,
            Position = UDim2.fromOffset(184, 28),
            Size = UDim2.fromOffset(18, 106),
            ClipsDescendants = true,
            ZIndex = 742,
            Parent = popup,
        }) :: Frame
        ...
        local hexBox = create("TextBox", {
            ...
            Font = Enum.Font.GothamMedium,
            TextSize = 10,
            Position = UDim2.fromOffset(38, 144),
        ...
        local rgbBox = create("TextBox", {
            ...
            Font = Enum.Font.GothamMedium,
            TextSize = 9,
            Position = UDim2.fromOffset(120, 144),
        ...
        if not continuous then
            local applyButton = create("TextButton", {
                ...
                TextSize = 10,
                AutoButtonColor = false,
                Position = UDim2.fromOffset(0, controlsY + 36),
                Size = UDim2.new(1, 0, 0, 30),
                ZIndex = 742,
                Parent = popup,
            }) :: TextButton
...
function Tab:SelectSubTab(owner: AnyTable): AnyTable
    ...
    owner.Columns.Position = UDim2.new(
        ...
        ResponsiveController:Scale(self.Window, 3),
        0,
        ResponsiveController:Scale(self.Window, TargetDesign.Content.VerticalPadding)
    )
    tween(owner.Columns, {
        Position = UDim2.new(0.5, 0, 0, ResponsiveController:Scale(self.Window, TargetDesign.Content.VerticalPadding)),
    }, Motion.SubtabSelect)
    SubtabController:Reveal(self, owner)
...
function Tab:_updateCanvas()
    task.defer(function()
        ...
        self.Scroll.CanvasSize = UDim2.fromOffset(0, height + ResponsiveController:Scale(self.Window, TargetDesign.Content.VerticalPadding * 2))
    end)
end
...
function Tab:_applyContentWidth()
    ...
    local available = math.max(self.Scroll.AbsoluteSize.X - d(TargetDesign.Content.HorizontalPadding * 2), 1)
    ...
    self.Columns.Position = UDim2.new(0.5, 0, 0, d(TargetDesign.Content.VerticalPadding))
...
function Tab:CreateSection(config: (NavigationOptions | string)?): AnyTable
    ...
    padding(frame, 9, 7, 9, 9)
    local frameLayout = list(frame, Enum.FillDirection.Vertical, 0)

    local heading = create("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 0, config.Description and 38 or 27),
        LayoutOrder = 1,
        Parent = frame,
    }) :: Frame
    local titleOffset = 0
    if config.Icon then
        ...
        if icon then
            ...
            titleOffset = icon.Size.X.Offset + 7
        end
    end
    local title = makeText(heading, section.Title, TargetDesign.Typography.Section, Theme.Text, "bold")
    title.Position = UDim2.fromOffset(titleOffset, 0)
    title.Size = UDim2.new(1, -titleOffset, 0, 20)
    if config.Description then
        local description = makeText(heading, tostring(config.Description), TargetDesign.Typography.Description, Theme.Muted)
        description.Position = UDim2.fromOffset(titleOffset, 19)
        description.Size = UDim2.new(1, -titleOffset, 0, 15)
    end
    local headingDivider = create("Frame", {
        BackgroundColor3 = Theme.Divider,
    ...
    ThemeController:Bind(headingDivider, "BackgroundColor3", "Divider")
...
function Tab:CreateSubTab(config: (NavigationOptions | string)?): AnyTable
    ...
    local columns = create("Frame", {
        BackgroundTransparency = 1,
        Position = UDim2.fromOffset(TargetDesign.Content.HorizontalPadding, TargetDesign.Content.VerticalPadding),
        Size = UDim2.new(1, -TargetDesign.Content.HorizontalPadding * 2, 0, 0),
        Parent = scroll,
    }) :: Frame
    local leftColumn = create("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(0.5, -TargetDesign.Content.ColumnGap * 0.5, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y,
        Parent = columns,
    }) :: Frame
    local rightColumn = create("Frame", {
        BackgroundTransparency = 1,
        Position = UDim2.new(0.5, TargetDesign.Content.ColumnGap * 0.5, 0, 0),
        Size = UDim2.new(0.5, -TargetDesign.Content.ColumnGap * 0.5, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y,
        Parent = columns,
    }) :: Frame
...
function Window:_setActiveTabVisual(tab: AnyTable, active: boolean)
    ...
    tween(tab.Button, {
        BackgroundColor3 = active and Theme.PressedSurface or Theme.Surface2,
        BackgroundTransparency = active and 0.66 or 1,
    }, Motion.Tab)
    tween(tab.ActiveBar, {
        BackgroundTransparency = active and 0 or 1,
        Size = active and UDim2.fromOffset(ResponsiveController:Scale(self, 3), ResponsiveController:Scale(self, 20))
...
function Window:CreateTab(config: (NavigationOptions | string)?): AnyTable
    ...
    local titleOffset = icon and (icon.Size.X.Offset + 25) or 13
    title.Position = UDim2.fromOffset(titleOffset, 0)
    title.Size = UDim2.new(1, -titleOffset - 10, 1, 0)
    ...
    local subTabBar = create("ScrollingFrame", {
        ...
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        Position = UDim2.fromOffset(TargetDesign.Content.HorizontalPadding, 5),
        Size = UDim2.new(1, -TargetDesign.Content.HorizontalPadding * 2, 0, 32),
        CanvasSize = UDim2.fromOffset(0, 0),
        ScrollBarThickness = 0,
    ...
    local subTabBarLayout = list(subTabBar, Enum.FillDirection.Horizontal, 6)
    ...
    scroll:SetAttribute("KronosScrollbarRole", "Content")
    ThemeController:Bind(scroll, "ScrollBarImageColor3", "Accent")
    local columns = create("Frame", {
        BackgroundTransparency = 1,
        Position = UDim2.fromOffset(TargetDesign.Content.HorizontalPadding, TargetDesign.Content.VerticalPadding),
...
local function makeUtilityButton(parent: Instance, iconName: string, order: number): TextButton
    ...
    corner(button, 4)
    stroke(button, Theme.Border, 0.98, 1, "Quiet")
    local icon = makeIcon(button, { Icon = iconName, IconSize = 15 }, "SubText")
    if icon then
...
function Window:_refreshHeaderContext()
    ...
    local tabTitle = activeTab and tostring(activeTab.Title) or "Overview"
    local pageTitle = tabTitle

    if activeSubTab and activeSubTab ~= activeTab then
        pageTitle = tostring(activeSubTab.Title)
    elseif activeTab then
        pageTitle = tostring(activeTab.DefaultSubTabName or activeTab.Title)
    end
    ...
    if breadcrumb and breadcrumb.Parent then
        if activeSubTab and activeSubTab ~= activeTab then
            breadcrumb.Text = tabTitle .. "   ›   " .. pageTitle
        else
            breadcrumb.Text = tabTitle
        end
    end

    local pageTitleLabel = self.PageTitleLabel
    if pageTitleLabel and pageTitleLabel.Parent then
        pageTitleLabel.Text = pageTitle
    end

    local sidebarValue = self.SidebarContextValue
    if sidebarValue and sidebarValue.Parent then
        sidebarValue.Text = pageTitle
    end
    local sidebarMeta = self.SidebarContextMeta
    if sidebarMeta and sidebarMeta.Parent then
        sidebarMeta.Text = activeSubTab and activeSubTab ~= activeTab
            and tabTitle .. " section"
            or tostring(self.Subtitle ~= "" and self.Subtitle or "Kronos workspace")
    end
end
...
function Window:_openHeaderContext()
    ...
    local maximumVisibleRows = TargetDesign.Popup.MaximumVisibleOptions
    local rowHeight = TargetDesign.Popup.OptionHeight
    local rowGap = 2
    local outerPadding = TargetDesign.Popup.Padding
    ...
    local popupWidth = TargetDesign.Popup.DropdownWidth
    ...
    for index, entry in ipairs(entries) do
        ...
        corner(row, TargetDesign.Control.Radius)
        ...
        local rowLabel = makeText(
            row,
            entry.Label,
            TargetDesign.Typography.Control,
            selected and Theme.Text or Theme.SubText,
            selected and "bold" or nil
        )
        ...
        local selectedIcon = makeIcon(row, { Icon = "check", IconSize = 12 }, "Accent")
...
function Window:_makeHeader(config: WindowConfig)
    ...
    corner(logo, 2)
    local logoStroke = stroke(logo, Theme.AccentSoft, 1, 1)
    ...
    local titleOffset = hasLogo and 50 or 18
    ...
    subtitle.Size = UDim2.fromOffset(hasLogo and 176 or 206, Metrics.Header)
    subtitle.ZIndex = 29
    ...
    local contextStroke = stroke(contextButton, Theme.Border, 0.93, 1, "Quiet")
    ...
    contextLabel.Position = UDim2.fromOffset(14, 0)
    contextLabel.Size = UDim2.new(1, -42, 1, 0)
    ...
    if contextChevron then
        ...
        contextChevron.Position = UDim2.new(1, -11, 0.5, 0)
        contextChevron.ZIndex = 32
    end

    local profileButton = makeUtilityButton(header, "settings-2", 1)
    ...
    local navigationButton = create("TextButton", {
        ...
        AutoButtonColor = false,
        AnchorPoint = Vector2.new(0, 0.5),
        Position = UDim2.fromOffset(Metrics.Sidebar + 16, math.floor(Metrics.Header * 0.5)),
        Size = UDim2.fromOffset(TargetDesign.Header.UtilitySize, TargetDesign.Header.UtilitySize),
        ZIndex = 32,
        Parent = header,
    }) :: TextButton
    corner(navigationButton, 4)
    stroke(navigationButton, Theme.Border, 0.98, 1, "Quiet")
    ThemeController:Bind(navigationButton, "BackgroundColor3", "Surface2")
    local navigationIcon = makeIcon(navigationButton, { Icon = "panel-left", IconSize = 15 }, "Muted")
    ...
    for _, button in ipairs({ profileButton, presetsButton, minimizeButton, closeButton }) do
        addConnection(
            ...
            button.MouseEnter:Connect(function()
                ...
                tween(button, { BackgroundTransparency = accentUtility and 0.12 or 0.72 }, Motion.HoverIn)
        ...
        addConnection(
            ...
            button.MouseLeave:Connect(function()
                local accentUtility = button:GetAttribute("KronosAccentUtility") == true
                tween(button, { BackgroundTransparency = accentUtility and 0.22 or 1 }, Motion.HoverOut)
                local icon = button:FindFirstChild("UtilityIcon")
                if icon then
    ...
    addConnection(self, contextButton.MouseEnter:Connect(function()
        tween(contextButton, { BackgroundTransparency = 0.58 }, Motion.HoverIn)
        tween(contextStroke, { Transparency = BorderController:Resolve(0.84, "Hover") }, Motion.HoverIn)
    end))
    addConnection(self, contextButton.MouseLeave:Connect(function()
        ...
        tween(contextStroke, { Transparency = BorderController:Resolve(0.93, "Quiet") }, Motion.HoverOut)
    end))
...
function Window:ApplyResponsive(animateNavigation: boolean?)
    ...
    local expandedThreshold = layoutMode == "MobileLandscape" and 500 or 620
    ...
    if expanded then
        local minimumSidebar = math.max(d(166), 92)
        ...
        local minimumSidebar = math.max(d(42), 34)
    ...
    local minimumHeader = math.max(d(46), 36)
    ...
    if self.HeaderContext then
        local headerContentWidth = math.max(width - sidebarWidth - d(64), 1)
        local desiredContextWidth = math.min(d(Metrics.HeaderContextWidth), math.max(headerContentWidth - d(128), d(170)))
        ...
        local contextPositionTarget = UDim2.fromOffset(
            math.floor(sidebarWidth + d(54) + desiredContextWidth * 0.5 + 0.5),
        ...
        self.HeaderContext.Visible = self.HeaderContextEnabled ~= false
            ...
            and headerContentWidth >= d(300)
        if self.NavigationButton then
            local navigationPositionTarget =
                UDim2.fromOffset(sidebarWidth + d(10), math.floor(headerHeight * 0.5 + 0.5))
            if animateLayout then
                tween(
    ...
    if self.HeaderTitle then
        self.HeaderTitle.Size = UDim2.fromOffset(
            math.max(sidebarWidth - d(72), d(112)),
            headerHeight
        )
    end

    local searchVisible = self.SearchEnabled ~= false and expanded
    local contextVisible = self.ShowSidebarContext ~= false and expanded
    self.SearchBox.Visible = searchVisible
    self.SearchBox.PlaceholderText = "Search"
    self.SearchBox.TextXAlignment = Enum.TextXAlignment.Left
    self.SearchBox.Size = UDim2.new(
        1,
        -d(TargetDesign.Navigation.HorizontalPadding * 2),
        0,
        d(TargetDesign.Navigation.SearchHeight)
    )
    self.SearchBox.Position = UDim2.fromOffset(
        d(TargetDesign.Navigation.HorizontalPadding),
        d(12)
    )
    self.SearchIcon.Visible = searchVisible

    if self.SidebarContext then
        self.SidebarContext.Visible = contextVisible
        local contextTop = searchVisible
            and d(12 + TargetDesign.Navigation.SearchHeight + TargetDesign.Navigation.ContextGap)
            or d(12)
        self.SidebarContext.Position = UDim2.fromOffset(
            d(TargetDesign.Navigation.HorizontalPadding),
            contextTop
        )
        self.SidebarContext.Size = UDim2.new(
            1,
            -d(TargetDesign.Navigation.HorizontalPadding * 2),
            0,
            d(TargetDesign.Navigation.ContextHeight)
        )
    end
    ...
    for _, tab in ipairs(self.Tabs) do
        ...
        if tab.IconLabel then
            tab.IconLabel.AnchorPoint = expanded and Vector2.new(0, 0.5) or Vector2.new(0.5, 0.5)
            tab.IconLabel.Position = expanded
                and UDim2.new(0, d(16), 0.5, 0)
    ...
    if self.SidebarFooter then
        self.SidebarFooter.Visible = self.ShowSidebarProfile == true and expanded
    end
    ...
    if self.NavigationScroll then
        local navigationTop = d(12)
        if searchVisible then
            navigationTop += d(TargetDesign.Navigation.SearchHeight + TargetDesign.Navigation.ContextGap)
        end
        if contextVisible then
            navigationTop += d(TargetDesign.Navigation.ContextHeight + TargetDesign.Navigation.ContextGap)
        end
        local reservedBottom = self.ShowSidebarProfile and expanded and d(64) or d(14)
        self.NavigationScroll.Position = UDim2.fromOffset(
            d(TargetDesign.Navigation.HorizontalPadding),
            navigationTop
        )
    ...
    local twoColumnThreshold = layoutMode == "MobileLandscape" and 470 or 540
    local minimumContentHeight = layoutMode == "MobileLandscape" and 235 or 270
    ...
    if self.SidePanel and self.SidePanel.Parent then
        ...
        local designPanelWidth = isPresets and 236 or (isProfile and 246 or 286)
        ...
        if isProfile or isPresets or isSettings then
            local desiredHeight = d(isProfile and 238 or (isPresets and 286 or 366))
...
function Window:ResetInterfaceLayout(): AnyTable
    ...
    self.Root.Position = UDim2.fromOffset(
        math.floor(topLeftInset.X + usableWidth * TargetDesign.Window.Center.X + 0.5),
        math.floor(topLeftInset.Y + usableHeight * TargetDesign.Window.Center.Y + 0.5)
    )
...
function FloatingWidgetController:Create(window: AnyTable, config: FloatingWidgetOptions): AnyTable
    ...
    local body = create("Frame", {
        BackgroundTransparency = 1,
        Position = UDim2.fromOffset(0, TargetDesign.Content.PageContextHeight),
        Size = UDim2.new(1, 0, 1, -TargetDesign.Content.PageContextHeight),
        ZIndex = root.ZIndex + 1,
        Parent = root,
    }) :: Frame
    widget.Root = root
...
local function buildWindow(library: AnyTable, config: WindowConfig): AnyTable
    ...
    baseWidth = math.max(finiteNumber(baseWidth, defaultWindowSize.X), 360)
    ...
    local window = setmetatable({
        ...
        -- The V2.1 CURRENT VIEW card was a current-only invention. V2.3 keeps
    ...
    local mainStroke = stroke(main, Theme.Border, 0.9, 1, "MainShellOuter")
    ThemeController:Bind(mainStroke, "Color", "Border")
    local innerStroke = stroke(main, Theme.InnerHighlight, 0.95, 1, "MainShellInner")
    ...
    local shellBorderStroke = stroke(shellBorder, Theme.Border, 0.8, 1, "MainShellOuter")
    ...
    local shellInnerStroke = stroke(shellInnerBorder, Theme.InnerHighlight, 0.96, 1, "MainShellInner")
    ...
    window.ShadowOuterTransparency = 0.95
    window.ShadowInnerTransparency = 0.89
    ...
    local searchBox = create("TextBox", {
        ...
        TextColor3 = Theme.Text,
        Font = Enum.Font.GothamMedium,
        TextSize = TargetDesign.Typography.Sidebar,
        TextXAlignment = Enum.TextXAlignment.Left,
        Position = UDim2.fromOffset(TargetDesign.Navigation.HorizontalPadding, 12),
        Size = UDim2.new(1, -TargetDesign.Navigation.HorizontalPadding * 2, 0, TargetDesign.Navigation.SearchHeight),
        ZIndex = 15,
        Parent = sidebar,
    }) :: TextBox
    ThemeController:Bind(searchBox, "BackgroundColor3", "Surface2")
    corner(searchBox, 3)
    local searchStroke = stroke(searchBox, Theme.Border, 0.9, 1, "Quiet")
    ...
    -- A compact contextual region gives the sidebar a real top/middle/bottom
    -- hierarchy and uses live navigation data rather than decorative filler.
    local contextCard = create("Frame", {
        Name = "SidebarContext",
        BackgroundColor3 = Theme.Surface,
        BackgroundTransparency = 0.28,
        BorderSizePixel = 0,
        Position = UDim2.fromOffset(
            TargetDesign.Navigation.HorizontalPadding,
            12 + TargetDesign.Navigation.SearchHeight + TargetDesign.Navigation.ContextGap
        ),
        Size = UDim2.new(
            1,
            -TargetDesign.Navigation.HorizontalPadding * 2,
            0,
            TargetDesign.Navigation.ContextHeight
        ),
        Visible = window.ShowSidebarContext,
        ZIndex = 15,
        Parent = sidebar,
    }) :: Frame
    ThemeController:Bind(contextCard, "BackgroundColor3", "Surface")
    corner(contextCard, 5)
    local contextCardStroke = stroke(contextCard, Theme.Border, 0.88, 1, "Quiet")
    ThemeController:Bind(contextCardStroke, "Color", "Border")
    local contextIconPlate = create("Frame", {
        BackgroundColor3 = Theme.Accent,
    ...
    ThemeController:Bind(contextIconPlate, "BackgroundColor3", "Accent")
    corner(contextIconPlate, 5)
    local contextIcon = makeIcon(contextIconPlate, { Icon = "pin", IconSize = 15 }, "Accent")
    if contextIcon then
        contextIcon.AnchorPoint = Vector2.new(0.5, 0.5)
        contextIcon.Position = UDim2.fromScale(0.5, 0.5)
        contextIcon.ZIndex = 17
    end
    local contextCaption = makeText(
        contextCard,
        "CURRENT VIEW",
        TargetDesign.Typography.Metadata,
        Theme.Muted,
        "bold"
    )
    contextCaption.Position = UDim2.fromOffset(51, 9)
    contextCaption.Size = UDim2.new(1, -61, 0, 14)
    contextCaption.ZIndex = 16
    local contextValue = makeText(
        contextCard,
        "Overview",
        TargetDesign.Typography.Sidebar,
        Theme.Text,
        "bold"
    )
    contextValue.Position = UDim2.fromOffset(51, 24)
    contextValue.Size = UDim2.new(1, -61, 0, 20)
    contextValue.TextTruncate = Enum.TextTruncate.AtEnd
    contextValue.ZIndex = 16
    local contextMetaText = tostring(config.Subtitle or config.SubTitle or "")
    if contextMetaText == "" then
        contextMetaText = "Kronos workspace"
    end
    local contextMeta = makeText(
        contextCard,
        contextMetaText,
        TargetDesign.Typography.Metadata,
        Theme.SubText
    )
    contextMeta.Position = UDim2.fromOffset(12, 51)
    contextMeta.Size = UDim2.new(1, -24, 0, 15)
    contextMeta.TextTruncate = Enum.TextTruncate.AtEnd
    contextMeta.ZIndex = 16

    local navigationTop = 12
        + TargetDesign.Navigation.SearchHeight
        + TargetDesign.Navigation.ContextGap
        + TargetDesign.Navigation.ContextHeight
        + TargetDesign.Navigation.ContextGap
    local navScroll = create("ScrollingFrame", {
        ...
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        Position = UDim2.fromOffset(TargetDesign.Navigation.HorizontalPadding, navigationTop),
        Size = UDim2.new(
            1,
            -TargetDesign.Navigation.HorizontalPadding * 2,
            1,
            -(navigationTop + (window.ShowSidebarProfile and 62 or 14))
        ),
        CanvasSize = UDim2.fromOffset(0, 0),
        AutomaticCanvasSize = Enum.AutomaticSize.None,
        ScrollBarThickness = 0,
    ...
    navScroll:SetAttribute("KronosScrollbarRole", "Sidebar")
    ...
    local sidebarLayout = list(sidebarList, Enum.FillDirection.Vertical, TargetDesign.Navigation.RowGap)
    addConnection(
        window,
        sidebarLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            navScroll.CanvasSize = UDim2.fromOffset(0, sidebarLayout.AbsoluteContentSize.Y + TargetDesign.Navigation.RowGap)
        end)
    )
    local footer = create("TextButton", {
        ...
        Text = "",
        AutoButtonColor = false,
        Position = UDim2.new(0, TargetDesign.Navigation.HorizontalPadding, 1, -(TargetDesign.Navigation.FooterHeight + 12)),
        Size = UDim2.new(1, -TargetDesign.Navigation.HorizontalPadding * 2, 0, TargetDesign.Navigation.FooterHeight),
    ...
    corner(footer, 5)
    stroke(footer, Theme.Border, 0.84, 1, "Control")
    local footerIcon = makeIcon(footer, { Icon = "user", IconSize = 15 }, "Accent")
    assert(footerIcon, "Required Lucide profile icon is unavailable")
    footerIcon.AnchorPoint = Vector2.new(0, 0.5)
    footerIcon.Position = UDim2.new(0, 14, 0.5, 0)
    footerIcon.ZIndex = 16
    local footerLabel = makeText(
        footer,
        LocalPlayer and LocalPlayer.DisplayName or "Profile",
        TargetDesign.Typography.Sidebar,
        Theme.SubText,
        "medium"
    )
    footerLabel.Position = UDim2.fromOffset(39, 0)
    footerLabel.Size = UDim2.new(1, -48, 1, 0)
    ...
    ThemeController:Bind(content, "BackgroundColor3", "Surface")
    ...
    ThemeController:Bind(pageContext, "BackgroundColor3", "BackgroundSoft")
    local pageContextDivider = create("Frame", {
        BackgroundColor3 = Theme.Divider,
        BackgroundTransparency = 0.74,
        BorderSizePixel = 0,
        AnchorPoint = Vector2.new(0, 1),
        Position = UDim2.new(0, TargetDesign.Content.HorizontalPadding, 1, 0),
        Size = UDim2.new(1, -TargetDesign.Content.HorizontalPadding * 2, 0, 1),
        ZIndex = 15,
        Parent = pageContext,
    }) :: Frame
    ThemeController:Bind(pageContextDivider, "BackgroundColor3", "Divider")

    local breadcrumb = makeText(
        pageContext,
        "Overview",
        TargetDesign.Typography.Breadcrumb,
        Theme.Muted,
        "medium"
    )
    breadcrumb.Name = "Breadcrumb"
    breadcrumb.Position = UDim2.fromOffset(TargetDesign.Content.HorizontalPadding, 5)
    breadcrumb.Size = UDim2.new(1, -TargetDesign.Content.HorizontalPadding * 2, 0, 14)
    breadcrumb.ZIndex = 15

    local pageTitle = makeText(
        pageContext,
        "Overview",
        TargetDesign.Typography.PageTitle,
        Theme.Text,
        "bold"
    )
    pageTitle.Name = "PageTitle"
    pageTitle.Position = UDim2.fromOffset(TargetDesign.Content.HorizontalPadding, 20)
    pageTitle.Size = UDim2.new(1, -TargetDesign.Content.HorizontalPadding * 2, 0, 22)
    pageTitle.ZIndex = 15
    ...
    local emptySearch = makeText(content, "No matching tabs or controls", TargetDesign.Typography.Control, Theme.Muted, "medium")
    ...
    window.SidebarContext = contextCard
    window.SidebarContextValue = contextValue
    window.SidebarContextMeta = contextMeta
    ...
    window.PageTitleLabel = pageTitle
